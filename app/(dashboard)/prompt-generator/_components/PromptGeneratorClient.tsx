...
import { compileGeneratorPrompt } from "@/lib/prompt-generator-compiler"
...
import { PromptGeneratorPreview } from "./PromptGeneratorPreview"

const DEFAULT_FORM: PromptGeneratorFormData = {
  taskCategory:    "",
  taskDescription: "",
  language:        "",
  codeContext:     "",
...
export function PromptGeneratorClient() {
  ...
  const [formData,       setFormData]       = useState<PromptGeneratorFormData>(DEFAULT_FORM)
  ...
  const compiledPrompt = useMemo(() => compileGeneratorPrompt(formData), [formData])
  ...
  const handleFormChange = useCallback((delta: Partial<PromptGeneratorFormData>) => {
  ...
  const handleDownload = useCallback(() => {
    ...
    const isDebug = formData.taskCategory === "Debug & Error Fix"
    a.href        = url
    a.download    = enhancedPrompt
      ? (isDebug ? "debug-prompt-enhanced.md" : "prompt-enhanced.md")
      : (isDebug ? "debug-prompt.md"          : "prompt-output.md")
    a.click()
    URL.revokeObjectURL(url)
    showToast("Markdown file downloaded!")
  }, [activePrompt, enhancedPrompt, formData.taskCategory, showToast])
    ...
    <div className="bg-background text-on-surface h-screen flex overflow-hidden">
      ...
      <main className="md:ml-[280px] flex-1 flex flex-col h-full bg-surface-dim overflow-hidden">
        ...
        <div className="flex-1 flex flex-col xl:flex-row overflow-hidden">
          ...
          <PromptGeneratorPreview
            prompt={activePrompt}
            isGenerated={isGenerated}
            isGenerating={isGenerating}
            isEnhanced={enhancedPrompt !== null}
            isEnhancing={isEnhancing}
            isLean={formData.promptMode === "lean"}
            onCopy={handleCopy}
